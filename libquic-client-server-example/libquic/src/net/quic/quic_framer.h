...
//当接收到frame帧信息，解包后进行相应的回调处理  QuicFramer类中包含该类成员
class NET_EXPORT_PRIVATE QuicFramerVisitorInterface {
 public:
  ...
  virtual void OnPacket() = 0; //QuicFramer::ProcessPacket中执行
  ...
  //QuicDispatcher::OnUnauthenticatedPublicHeader 或者 QuicConnection::OnUnauthenticatedPublicHeader
...
/*
解包参考:QuicDispatcher::ProcessPacket

QuicDispatcher包含framer_成员
QuicConnection类包含QuicFramer framer_成员
*/
class NET_EXPORT_PRIVATE QuicFramer { //quic帧组包、解包信息类， 
 ...
 private:
  ...
  //quic数据解析读取相关,参考QuicFramer::ProcessPacket
  scoped_ptr<QuicDataReader> reader_;
  //当接收到frame帧信息，解包后进行相应的回调处理接口  
  //客户端赋值为QuicConnection类，见QuicConnection::QuicConnection   QuicConnection包含QuicFramer framer_
  //服务端赋值为QuicFramerVisitor类，见QuicDispatcher::QuicDispatcher  QuicDispatcher类包含QuicFramer framer_成员
  QuicFramerVisitorInterface* visitor_; //set_visitor接口赋值  QuicFramerVisitor
